...
def isWinner(x, nums):
    ...
    max_limit = max(nums)  # Find the maximum number in nums to determine the range for the sieve

    # Initialize a list for the sieve of Eratosthenes
    sieve = [True] * (max(max_limit + 1, 2))
    
    # Implement the sieve of Eratosthenes to find all prime numbers up to max_limit
    ...
    for index in range(len(sieve)):
        if sieve[index]:
            prime_count += 1
        sieve[index] = prime_count  # Store the cumulative count of primes up to each index

    maria_wins = 0  # Counter for the number of rounds Maria wins
    for round_limit in nums:
        maria_wins += sieve[round_limit] % 2 == 1  # Maria wins if the number of primes is odd
